...
import { StyleSheet, Share, TouchableOpacity, Text, RefreshControl, ScrollView, View } from 'react-native';
...
export default function Khatam() {
  ...
  const shareData = async () => {
    try {
      await Share.share({
        message: "Join the Khatam/Tasbeeh Reading using this code " + JSON.stringify(code),
      });
    } catch (error) {
      alert(error.message);
    }
      ...
      {isDataRetrieved ?
        <>
          ...
          {username === data.creator ?
            <TouchableOpacity style={styles.deleteButton} onPress={deleteRoomConfirmation}>
